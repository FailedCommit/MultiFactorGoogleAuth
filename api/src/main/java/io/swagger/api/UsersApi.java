...
public interface UsersApi {
    ...
    @Operation(summary = "Enable or Disable 2 Factor Authentication for the User", description = "", security = {
        @SecurityRequirement(name = "basicAuth")    }, tags={ "users" })
    @ApiResponses(value = { 
        ...
        @ApiResponse(responseCode = "200", description = "unexpected error", content = @Content(schema = @Schema(implementation = Error.class))) })
    @RequestMapping(value = "/users/{userId}/2fa",
        produces = { "application/json" }, 
        method = RequestMethod.POST)
    ResponseEntity<User> enable2FA(@Parameter(in = ParameterIn.PATH, description = "The id of the user to which 2fa is being enabled/disabled.", required=true, schema=@Schema()) @PathVariable("userId") String userId);
